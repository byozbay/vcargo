...
$base = new BaseModel();
$bId  = intval($_SESSION['branch_id'] ?? 0);
$dateFilter = $_GET['date'] ?? date('Y-m-d');
$statusFilter = $_GET['status'] ?? '';
$searchQ = $_GET['q'] ?? '';

// KPI counts (branch + today)
$counts = $base->query(
    "SELECT
        COUNT(*) AS total,
     ...
     FROM shipments WHERE branch_id=? AND is_active=1",
    [$bId]
)[0] ?? [];

// Shipments list with optional filters
$sql    = "SELECT s.shipment_id, s.tracking_no, s.sender_name, s.sender_phone,
...
if ($searchQ)      { $sql .= ' AND (s.tracking_no LIKE ? OR s.receiver_name LIKE ?)'; $params[] = "%$searchQ%"; $params[] = "%$searchQ%"; }
$sql .= ' ORDER BY s.created_at DESC LIMIT 100';
$shipments = $base->query($sql, $params);
...
$payStyle = [
    'SENDER_PAYS'   =>'background:#e8f1ff;color:#1b84ff;',
    'CARD'          =>'background:#e7f9f0;color:#0e8045;',
    'RECEIVER_PAYS' =>'background:#fff8ec;color:#e08b00;',
    'ACCOUNT'       =>'background:#f3e5f5;color:#8e24aa;',
];
$statStyle = [
    'in_transit' =>'background:#e8f1ff;color:#145dc0;',
    'in_storage' =>'background:#fff8ec;color:#e08b00;',
    'at_branch'  =>'background:#fff8ec;color:#e08b00;',
    'delivered'  =>'background:#e7f9f0;color:#0e8045;',
    'cancelled'  =>'background:#fff0f2;color:#c03060;',
...
<div style="padding:18px 26px 32px;">
    ...
    <div class="card mb-3" style="padding:16px 20px;">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-6 col-md-2">
                ...
                <input type="date" id="filterDate" value="<?= htmlspecialchars($dateFilter) ?>" name="date"
    ...
    <div class="card" style="padding:20px;">
            ...
            <table class="orders-table" id="shipmentTable">
                ...
                <tbody id="shipmentBody">
                    <?php if (empty($shipments)): ?>
                        <tr><td colspan="10" class="text-center" style="padding:32px;color:var(--text-muted);font-size:.82rem;">
                            <i class="bi bi-inbox" style="font-size:1.8rem;display:block;margin-bottom:8px;opacity:.4;"></i>
                            Kargo bulunamadı.
                        </td></tr>
                    <?php else: foreach ($shipments as $s): ?>
                        <tr data-status="<?= htmlspecialchars($s['status']) ?>" data-payment="<?= strtolower($s['payment_type']) ?>">
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="?page=shipment_detail&id=<?= $s['shipment_id'] ?>" class="icon-btn-circle" title="Detay" style="display:inline-flex;align-items:center;justify-content:center;text-decoration:none;">
                                        <i class="bi bi-eye" style="font-size:.8rem;"></i></a>
                                    <button class="icon-btn-circle" title="Barkod Bas"
                                        onclick="window.open('?page=barcode&id=<?= $s['shipment_id'] ?>','_blank')">
                                        <i class="bi bi-upc-scan" style="font-size:.8rem;"></i></button>
                                    <?php if (in_array($s['status'],['in_storage','at_branch'])): ?>
                                        <a href="?page=delivery&tracking=<?= urlencode($s['tracking_no']) ?>"
                                           class="icon-btn-circle" title="Teslim Et"
                                           style="display:inline-flex;align-items:center;justify-content:center;text-decoration:none;">
                                            <i class="bi bi-check2-circle" style="font-size:.8rem;color:#0e8045;"></i></a>
                                    <?php endif; ?>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="d-flex align-items-center justify-content-between mt-3 flex-wrap gap-2">
            <div style="font-size:.78rem;color:var(--text-muted);"><?= count($shipments) ?> kayıt gösteriliyor</div>
            <div class="d-flex gap-1">
